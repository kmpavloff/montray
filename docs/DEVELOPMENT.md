...
The release workflow publishes a self-contained `win-x64` zip and attaches it to a GitHub Release. The custom archive contains only the runtime executable, license files, and installation instructions. GitHub also shows automatic `Source code` archives for every tag; those are generated by GitHub and are not the app distribution.
